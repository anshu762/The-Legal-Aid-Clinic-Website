    ...
    volunteers = await prisma.user.findMany({
      where: {
        ...
        advisorProfile: {
          ...
          isPaused: false, // Don't show inactive volunteers publicly
        }
      },
      select: {
        id: true,
    ...
    <div className="min-h-[calc(100vh-16rem)] bg-muted/20 py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="text-4xl font-extrabold text-foreground sm:text-5xl font-serif tracking-tight">Our Legal Volunteers</h1>
          <p className="mt-4 text-xl text-muted-foreground max-w-2xl mx-auto">
            Dedicated professionals donating their time to make justice accessible to everyone.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {volunteers.map((volunteer) => (
            <Card key={volunteer.id} className="overflow-hidden hover:shadow-lg transition-all duration-300 border-border group flex flex-col">
              <div className="h-24 bg-gradient-to-br from-primary/80 to-primary w-full" />
              <CardContent className="p-6 flex flex-col items-center flex-grow">
                {volunteer.image ? (
                  <div className="relative w-24 h-24 mb-4 rounded-full overflow-hidden border-4 border-muted -mt-16 bg-background">
                    <Image
                      src={volunteer.image}
                  ...
                  <div className="relative w-24 h-24 mb-4 rounded-full overflow-hidden border-4 border-muted -mt-16 bg-muted/50 flex items-center justify-center">
                    {/* Using DiceBear Micah (Premium Illustration Style) */}
                    <img
                      src={`https://api.dicebear.com/9.x/micah/svg?seed=${volunteer.fullName.replace(" ", "")}&backgroundColor=transparent`}
                      alt={volunteer.fullName}
